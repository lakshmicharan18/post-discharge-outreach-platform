...
The trusted request context carries `user_id`, `hospital_id`, and `role`. Only identity resolution accesses users globally. The development adapter derives hospital and role from stored users; a future authentication adapter must verify identity before constructing this context. There is no caller-controlled tenant selector.
...
Platform HospitalRepository is separate, checks PLATFORM_ADMIN, and only exposes hospital metadata. Hospital creation is a foundation operation; onboarding workflows and user administration are deferred. Hospital roles currently share clinical list/get/create permissions; the schema and context establish the role vocabulary without claiming complete workflow RBAC.
...
The Next.js shell contains no clinical records. All data authorization remains in the backend. No future workflow is implemented in the reserved packages.
