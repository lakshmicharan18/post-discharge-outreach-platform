...
Authenticated clinical users can open `/queue` to view capacity, queue depth, deadline pressure, task priority explanations, and outreach task state. Hospital administrators and campaign managers can use the prototype reservation controls; clinical reviewers have read-only visibility. Milestone 5 schedules and reserves work but does not execute calls. Call outcomes, retries, callbacks, manual-follow-up processing, worker crash recovery, and queue simulation remain deferred to Milestone 6.
