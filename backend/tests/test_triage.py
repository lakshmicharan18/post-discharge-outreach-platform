...
@pytest.mark.parametrize(
    "classification", ["ROUTINE", "CONCERNING", "URGENT", "INSUFFICIENT_INFORMATION"]
)
    ...
    model = DeterministicFakeModel(
        {
            ...
            "patient_reported_findings": ["reported symptom"],
            "clinical_context_facts": ["discharge record fact"],
            "protocol_references": [],
            "reasoning_summary": "Short auditable summary.",
            "recommended_next_action": "HUMAN_REVIEW",
            "confidence": 0.5,
            "requires_human_review": classification == "INSUFFICIENT_INFORMATION",
        }
    )
    record = await ClinicalTriageService(session, context(1), model).assess(
...
async def test_malformed_output_and_foreign_session_fail_safely(client, session):
    ...
    with pytest.raises(Exception):
        await ClinicalTriageService(session, context(1), DeterministicFakeModel({})).assess(
            session_id, datetime.now(timezone.utc)
        )
    with pytest.raises(APIError):
        await ClinicalTriageService(session, context(2), DeterministicFakeModel({})).assess(
            session_id, datetime.now(timezone.utc)
        )
