...
type OutreachTask = { id: string; state: string };
...
export default function CampaignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const load = useCallback(async (nextOffset = offset, nextFilter = filter) => {
    ...
    try {
      ...
      const [nextCampaign, nextEstimate, nextEligibility, nextTasks] = await Promise.all([
        api<Campaign>(`campaigns/${id}`), api<Estimate>(`campaigns/${id}/estimate`), api<EligibilityPage>(`campaigns/${id}/eligibility?${query}`), api<OutreachTask[]>(`campaigns/${id}/outreach-tasks?limit=100`),
      ]);
      setCampaign(nextCampaign); setEstimate(nextEstimate); setEligibility(nextEligibility); setOutreachTasks(nextTasks);
  ...
  return <main><Navigation />
    ...
    {campaign && <><p className="eyebrow">CAMPAIGN OPERATIONS</p><h1>{campaign.name}</h1><p><span className="badge">{campaign.status}</span></p>
      ...
      <section><div className="section-heading"><h2>Outreach queue</h2><Link href="/queue">Open queue operations</Link></div><div className="metric-grid"><p><strong>{outreachTasks.length}</strong><span>Tasks</span></p><p><strong>{outreachTasks.filter((task) => task.state === "PENDING").length}</strong><span>Pending</span></p><p><strong>{outreachTasks.filter((task) => task.state === "SCHEDULED").length}</strong><span>Scheduled</span></p><p><strong>{outreachTasks.filter((task) => task.state === "COMPLETED").length}</strong><span>Completed</span></p></div></section>
      ...
      <p className="muted">Milestone 5 schedules and reserves outreach work. It does not execute calls or process outcomes.</p>
    </>}
  </main>;
}
